library("meta")
library(metafor)
data=read.csv("anygrade.csv",header = T)
...
mg1v <- metagen(log(OR),lower = log(L), upper = log(U),
                ...
                common = T,subgroup =  AEType1, print.subgroup.name = FALSE)
summary(mg1v)
pdf("anygrade.pdf",
    width = 22 / 2.54,   
    height = 54 / 2.54, 
...
dev.off()
metabias(ORR, k.min = 7, method.bias = "Egger")
dev.off()

#1轮廓增强漏斗图
tiff('grade3.tiff', width=5000,height=12200,res=300)
funnel(ORR, common = T, pch =16,
       contour.levels = c(0.9, 0.95, 0.99),
       col.contour = c("darkgray", "gray", "lightgray"))
legend(0.45, 0.35, #p值坐标
       c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"),bty="n")
dev.off
#剪补法
pdf("grade3 correction.pdf",
    width = 22 / 2.54,  
    height = 48 / 2.54, 
    paper = "special",  
    onefile = FALSE)    
...
funnel(tf1, common = T, pch =16,
       contour.levels = c(0.9, 0.95, 0.99),studlab = TRUE,
       col.contour = c("darkgray", "gray", "lightgray"))
legend(0.45, 0.35, 
       c("0.1 > p > 0.05", "0.05 > p > 0.01", "< 0.01"),bty="n")
tiff('grade3 轮廓增强漏斗图剪补后3.12.tiff', width=5000,height=12200,res=300)
funnel(tf1, common = T, pch =16,
       contour.levels = c(0.9, 0.95, 0.99),
       col.contour = c("darkgray", "gray", "lightgray"))
legend(0.45, 0.35, 
...
pdf("anygrade Sensitivity analysis.pdf",
    width = 23 / 2.54,   
    height = 50 / 2.54,
    paper = "special",   
    onefile = FALSE)     
